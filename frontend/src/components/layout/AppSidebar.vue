...
interface NavItem {
  ...
  externalUrl?: string  // 外部链接,存在时用 <a> 渲染并新标签打开
  children?: NavItem[]
  /**
...
// ProviderIcon: 上游渠道商（建筑/商铺），语义贴合"号商"维度
const ProviderIcon = {
  render: () =>
    h(
      'svg',
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/recharge', label: t('nav.recharge'), icon: CreditCardIcon, externalUrl: 'https://catfk.com/shop/F2HWMY7T', hideInSimpleMode: true },
  ...
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon, hideInSimpleMode: true },
    {
      ...
      children: [
        ...
        { path: '/admin/channels/providers', label: t('nav.channelProviders'), icon: ProviderIcon },
