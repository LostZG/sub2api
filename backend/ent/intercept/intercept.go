...
// The ChannelProviderFunc type is an adapter to allow the use of ordinary function as a Querier.
type ChannelProviderFunc func(context.Context, *ent.ChannelProviderQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ChannelProviderFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ChannelProviderQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ChannelProviderQuery", q)
}

// The TraverseChannelProvider type is an adapter to allow the use of ordinary function as Traverser.
type TraverseChannelProvider func(context.Context, *ent.ChannelProviderQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseChannelProvider) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseChannelProvider) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ChannelProviderQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ChannelProviderQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ChannelProviderQuery:
		return &query[*ent.ChannelProviderQuery, predicate.ChannelProvider, channelprovider.OrderOption]{typ: ent.TypeChannelProvider, tq: q}, nil
